...
def preview_every(request: HttpRequest) -> HttpResponse:
    ...
    if product_ids_raw:
        ...
        try:
            ...
            return wrap_data(
                {'products': db_interface.product.preview_by_ids(product_ids_list)}
            )
        except ValueError:
...
def full_by_id(request: HttpRequest, product_id: int) -> HttpResponse:
    try:
        product = db_interface.product.full_by_id(product_id)
